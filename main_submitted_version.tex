...
In Math 136, we look at two closely related discrete models. The first is purely combinatorial: graph $2$ - manifolds, where geodesics are shortest paths in the graph metric and
...
In this section, we use the discrete $2$ manifold used in Math 136. We discuss a combinatorial curvature that underlies graph-theoretic Gauss-Bonnet theorem. The goal here is twofold: (i) provide a definition of a finite $2$-manifold in the sense of Math 136 and (ii) justify the curvature formula 
...
The cycle condition for $S(v)$ encodes the local manifold requirement that a small neighborhood of $v$ has the topology of a $1$--sphere.  In Math 136, we discuss this for all closed triangulated surfaces: sphere, torus, projective plane, and higher-genus surfaces.
...
\]
since the line eleement along $C_r(v)$ in the cone model is $ds = r\, d\varphi$.  For a fixed number $N$ of equally spaced rays filling out a full circle, the average spacing between neighboring rays at radius $r$ is proportional to
\[
\frac{L(v,r)}{N} = \frac{\Theta(v) r}{N}.
\]
Compared to the flatt case, where $\Theta = 2\pi$, we see:
...
By Proposition~\ref{prop:cone-isometry}, a sufficiently small intrinsic neighborhood of $p$ is isometric to a Euclidean cone $C_{\Theta(p)}$ of total angle $\Theta(p)$ \cite{BobenkoSurisDDG}.  In this model, each geodesic ray from from $p$ corresponds to a straight ray with polar coordinates $(r,\varphi)$, where $\varphi\in[0,\Theta(p))$ is an angular parameter intrinsic to the cone.  Thus the space of initial geodesic directions at $p$ has total angular measure $\Theta(p)$ rather than $2\pi$. Positive curvature ($K(p)>0$) reduces the local angular space; negative curvature increases it.
...
Theoctahedron has $(V,E,F)=(6,12,8)$ with $\deg(v)=4$ at every vertex
\cite{KnillMath136Notes}:
\[
...
\]
Thiss mirrors the polyhedral situation, in which a vertex of negative angle defect produces $L(p,r) > 2\pi r$, reflecting the divergence of nearby geodesics \cite{BobenkoSurisDDG}.
...
\qquad D_G(v,2)<0,
\]
even when $v$ itself has degree $7$.  This shows that neighboring positive curvature has an impact on the growth of metric spheres, matching the behavior of polyhedral geodesic circles near a vertex of small negative defected when situated on a globally positively curfed surface.
...
Altogether, this makes $D_G(v,r)$  significant for interpreting geodesic behavior. It's able to measure the quantity controlled by geodesic deviation in the smooth theory: the rate at which nearby geodesics seperate as one moves outward from a point. 
...
This perspective is intuitive: discrete curvature furnishes a local stability descriptor for geodesic flow on finite surfaces, and indicates how predictable or divergent the evolution of neighboring states is within given discrete geometric environment. 
